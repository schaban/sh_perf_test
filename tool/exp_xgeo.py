...
class SkinSphere:
	...
	def estimate(self):
		d2x = xd.vecMag2(self.xgeo.getPntPos(self.imax[0]) - self.xgeo.getPntPos(self.imin[0]))
		d2y = xd.vecMag2(self.xgeo.getPntPos(self.imax[1]) - self.xgeo.getPntPos(self.imin[1]))
		d2z = xd.vecMag2(self.xgeo.getPntPos(self.imax[2]) - self.xgeo.getPntPos(self.imin[2]))
		minIdx = self.imin[0]
		maxIdx = self.imax[0]
		...
		self.radius = xd.vecMag(maxPos - self.center)

	def grow(self, pos):
		v = pos - self.center
		dist2 = xd.vecMag2(v)
		if dist2 > xd.sq(self.radius):
			dist = sqrt(dist2)
			r = (self.radius + dist) * 0.5
			s = xd.div0((r - self.radius), dist)
			self.radius = r
			self.center += v * s
...
class GeoExporter(xd.BaseExporter):
	def __init__(self):
	...
	def build(self, sop, bvhFlg = not True, skinSphFlg = False):
		...
		self.skinNodeNum = 0
		self.maxSkinWgt = 0
		self.skinNames = xh.getSkinNames(self.geo)
		if self.skinNames:
			self.skinIds = []
...
if __name__=="__main__":
	...
	outPath = exePath
	#outPath = r"D:/tmp/"
	outPath += r"\_test.xgeo"

	sop = xh.findSOPEXP()
	if not sop: sop = xh.findSkinNode()
	#if not sop: sop = hou.node('/obj/geo1/file1') #########
	print "Exporting from", sop.path()
